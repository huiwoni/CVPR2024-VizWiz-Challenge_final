...
######################################################################################################################## class end
    ...
    ############################################################################################################################################################################################ func start
        ...
        ######################################################### for start
            feats_, _ = self.model(self.transform(x), return_feats = True)
            feature_banks.append(feats_.detach())
        ######################################################### for end

        feature_bank = torch.stack(feature_banks, dim=1)

        feats_mask = torch.where(label_mask==True)[0]
        outputs_ema_masked = outputs_ema[feats_mask, :].detach()
        feats_masked = feature_bank[feats_mask, :, :]
    ...
    ############################################# func start
        feat_euc = (feature**2).sum(dim=2).unsqueeze(dim=2)
